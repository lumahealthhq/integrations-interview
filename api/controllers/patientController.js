...
exports.list_all_patients = (req, res) => {
    Patients.find({}, (err, patient) => {
        err ? res.status(500).send({message: "Failed to get patients information",error: err})
...
exports.add_a_patient = (req, res) => {
    ...
    new_patient.save((err, patient) => {
        err ? res.status(500).send({message: "Failed to add patient information",error: err})
...
exports.get_a_patient = (req, res) => {
    Patients.findOne({_id:req.params.patientId}, (err, patient) => {
        err ? res.status(500).send({message: "Failed to get patient information",error: err})
...
exports.update_a_patient = (req, res) => {
    Patients.findOneAndUpdate({_id: req.params.patientId}, req.body, {upsert: true, new: true}, (err, patient) => {
...
exports.delete_a_patient = (req, res) => {
    Patients.remove({
        ...
        err ? res.status(500).send({message: "Failed to delete patient information",error: err })
